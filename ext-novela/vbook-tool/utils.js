...
function checkDeviceConnect(ip, port, timeoutMs = 1500) {
    return new Promise((resolve) => {
        ...
        const checkPath = (port === 1122) ? '/getBookSources' : '/connect';
        const req = http.get({ hostname: ip, port, path: checkPath, timeout: timeoutMs }, (res) => {
            let data = '';
...
async function resolveVBookEndpoint(options = {}) {
    ...
    if (options.ip) {
        const result = await checkDeviceConnect(options.ip, port, options.timeoutMs || 1500);
        return { ip: options.ip, port, isLegado: result.isLegado || port === 1122, candidates: [result] };
    }
    ...
    for (let i = 0; i < ips.length; i++) {
        ...
        if (result.ok) {
            ...
            return { ip: result.ip, port, isLegado: result.isLegado || port === 1122, candidates: results };
        }
    }
